...
class GeneralizedCurvatureStateSpace : public SE2StateSpace
{
public:
    ...
    // Constructor
    GeneralizedCurvatureStateSpace(double turningRadius = 1.0, double curvature_dot = 0.2, double gamma = 1.0): rho_(turningRadius), sigma_(curvature_dot), gamma_(gamma)
    {
        std::cout << "using Generalized Curvature!" << std::endl;
...
protected:
    ...
    /** \brief max_curvature_derivative */
    double discretization_interval_ = 0.01;
};

/** \brief It is not used in HC, but it is needed for integrating OMPL*/
class GeneralizedCurvatureMotionValidator : public MotionValidator
{
public:
    GeneralizedCurvatureMotionValidator(SpaceInformation *si) : MotionValidator(si)
    {
        std::cout << "Motion Validator Constructor" << std::endl;
        defaultSettings();
    }
    GeneralizedCurvatureMotionValidator(const SpaceInformationPtr &si) : MotionValidator(si)
    {
        defaultSettings();
    }
    ~GeneralizedCurvatureMotionValidator() override = default;
    bool checkMotion(const State *s1, const State *s2) const override;
    bool checkMotion(const State *s1, const State *s2, std::pair<State *, double> &lastValid) const override;

private:
    GeneralizedCurvatureStateSpace *stateSpace_;
