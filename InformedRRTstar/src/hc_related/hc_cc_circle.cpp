#include "../../inc/utilities/hc_cc_circle.h"
...
double HC_CC_Circle::rs_circular_deflection(double delta) const
{
  ...
  // irregular rs-turn
    ...
    return (delta <= M_PI) ? delta : delta - TWO_PI;
}
...
bool HC_CC_Circle::cc_elementary_sharpness(const Configuration &q, double delta, double &sigma0) const
{
  ...
  {
    sigma0 = 4 * M_PI * pow(this->D1(0.5 * delta), 2) / pow(distance, 2);
    if (!this->left)
    {
      sigma0 = -sigma0;
    }
