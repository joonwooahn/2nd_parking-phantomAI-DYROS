...
public:
                            ...
                            Configuration **q2) const
  {
    ...
    if (c1.left && !c1.forward)
    {
      theta = angle - alpha;
      global_frame_change(c1.xc, c1.yc, theta, delta_x, delta_y, &x, &y);
      *q1 = new Configuration(x, y, theta + M_PI, 0);
      global_frame_change(c2.xc, c2.yc, theta, -delta_x, -delta_y, &x, &y);
      *q2 = new Configuration(x, y, theta + M_PI, 0);
    }
    if (!c1.left && c1.forward)
    {
      theta = angle - alpha;
                    ...
                    Configuration **q1, Configuration **q2) const
  {
    ...
    if (c1.left && c1.forward)
    {
      ...
      *q1 = new Configuration(x, y, theta + M_PI, c1.kappa);
                    ...
                    Configuration **q1, Configuration **q2) const
  {
    ...
    if (c1.left && !c1.forward)
    {
      ...
      *q1 = new Configuration(x, y, theta + M_PI, 0);
      global_frame_change(c2.xc, c2.yc, theta, delta_x2, -delta_y2, &x, &y);
      *q2 = new Configuration(x, y, theta + M_PI, c2.kappa);
    }
    if (!c1.left && c1.forward)
    {
      theta = angle - alpha;
...
{
  // cout <<"HC00_Reeds_Shepp_State_Space constructor" << endl;
...
HC_CC_RS_Path *HC00_Reeds_Shepp_State_Space::hc00_reeds_shepp(const hcState &state1, const hcState &state2) const
{
  ...
  // cout << "*HC00_Reeds_Shepp_State_Space::hc00_reeds_shepp" << endl;
  // cout << "start config: " << state1.x << " " << state1.y << " " << state1.theta << endl;
  // cout << "goal config: " << state2.x << " " << state2.y << " " << state2.theta << endl;

  // @MSK: 시작 및 도착 위치에 각각 원을 4개 생성.
...
double HC00_Reeds_Shepp_State_Space::get_distance(const hcState &state1, const hcState &state2) const
{
...
vector<Control> HC00_Reeds_Shepp_State_Space::get_controls(const hcState &state1, const hcState &state2) const
{
  // cout << "HC00_Reeds_Shepp_State_Space::get_controls" << endl;
  vector<Control> hc_rs_controls;
  //@MSK: Control 구조체는 {delta_s, kappa, sigma}로 구성.
  hc_rs_controls.reserve(10);
  HC_CC_RS_Path *p = this->hc00_reeds_shepp(state1, state2);
  // cout << "TYPE OF P: " << p->type << endl;
  switch (p->type)
  {
    case hc_cc_rs::E:
    ...
    case hc_cc_rs::TcTTcT:
      ...
      hc_turn_controls(*(p->cend), *(p->qi3), false, hc_rs_controls);
      // cout << "SIZE: " << hc_rs_controls.size() << endl;
      break;
    // ########################################################################
    case hc_cc_rs::TTT:
      cc_turn_controls(*(p->cstart), *(p->qi1), true, hc_rs_controls);
