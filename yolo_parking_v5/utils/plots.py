...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    center = (int((int(x[0])+int(x[2]))/2.0), int((int(x[1])+int(x[3]))/2.0))
    ...
    if label:
        ...
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 7.0, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 7.0, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
